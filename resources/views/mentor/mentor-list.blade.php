                ...
                <table class="display table" id="ul-contact-list" style="width:100%">
                    ...
                    @foreach($data as $data)
                        <tr>
                            <td>
                                @role('inkubator')
                                <a href="{{route('inkubator.profile-detail',$data->id)}}">
                                    <div class="ul-widget-app__profile-pic"><img class="profile-picture avatar-sm mb-2 rounded-circle img-fluid" src="{{ asset('img/mentor/profile/'.$data->profile->foto)}}" alt="Profil" />{{$data->profile->nama}}</div>
                                </a>
                                @endrole
                                @role('tenant')
                                <a href="{{route('tenant.profile-detail',$data->id)}}">
                                    <div class="ul-widget-app__profile-pic"><img class="profile-picture avatar-sm mb-2 rounded-circle img-fluid" src="{{ asset('img/mentor/profile/'.$data->profile->foto)}}" alt="Profil" />{{$data->profile->nama}}</div>
                                </a>
                                @endrole
                            </td>
                            <td>{{ $data->email }}</td>
                            <td>{{ $data->profile->kontak }}</td>
                            <td><a class="badge badge-primary m-2 p-2" href="#">Developer</a></td>
                            <td>20</td>
                            <td>{{ $data->profile->created_at->format("d M Y") }}</td>
                            <td>$320,800</td>
