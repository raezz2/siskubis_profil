			...
			<div class="tab-content" id="myIconTabContent" style="padding: 1rem 0 !important; ">
				<div class="tab-pane fade show active" id="homeIcon" role="tabpanel" aria-labelledby="home-icon-tab">
					<table class="display table" id="masuk" style="width:100%">
						...
						<tbody>
						@foreach($surat as $s)
							@if ($s->kepada == Auth::user()->id )
							<tr>
								<td>
								<a href="/inkubator/surat/{{ $s->id }}">
								...
								<td>{{ $s->created_at }}</td>
								<td><a class="ul-link-action text-success" href="/inkubator/surat/edit/{{ $s->id }}" data-toggle="tooltip" data-placement="top" title="Edit"><i class="i-Edit"></i></a><a class="ul-link-action text-danger mr-1" href="/inkubator/surat/delete/{{ $s->id }}" data-toggle="tooltip" data-placement="top" title="Want To Delete !!!"><i class="i-Eraser-2"></i></a></td>
							</tr>
							@endif
							@endforeach
						</tbody>
				...
				<div class="tab-pane fade" id="profileIcon" role="tabpanel" aria-labelledby="profile-icon-tab">
					...
					<tbody>
						<tr>
							...
							<td><a class="badge badge-danger m-2 p-2" href="#"></a></td>
							<td></td>
